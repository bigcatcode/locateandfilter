...
<h1><?php _e("Default settings","locateandfilter")?></h1>
<h2><?php _e("Map Language","locateandfilter")?></h2>
<table>
<tr style="display:none;">
<td><?php _e("Purchase Code","locateandfilter");?>:</td>	<td><input type="text" style="max-width:auto" size="55" name="locate-anything-option-license-key" value="<?php echo unserialize(get_option("locate-anything-option-license-key"));?>"><br>
	<?php //_e(" &nbsp;<a  target='_blank' href='#'>Get a License Key for only $4.99!</a> (Removes the 'Powered by LocateAndFilter' label)","locateandfilter")?>
</td>
</tr>
	<tr>
<td><?php _e("GoogleMaps Key (only if you use GoogleMaps)","locateandfilter");?>:</td>	<td><input type="text" name="locate-anything-option-googlemaps-key" value="<?php echo unserialize(get_option("locate-anything-option-googlemaps-key"));?>"></td>
</tr>

<tr class="deprecated">
	<td><?php _e("BingMaps Key (only if you use BingMaps)","locateandfilter");?>:</td>	<td><input type="text" name="locate-anything-option-bingmaps-key" value="<?php echo unserialize(get_option("locate-anything-option-bingmaps-key"));?>"></td>
</tr>

<tr>
<td><?php _e("Map Language","locateandfilter");?>:</td>
...
<h2><?php _e("What do you want load JS? and add to Layers Control","locateandfilter")?></h2>
	<ul>
		<li>
...
<h2><?php _e("What do you want to localize?","locateandfilter")?></h2>
	<ul>
		<li>
	...
	<h2><?php _e("Additional Fields","locateandfilter")?></h2>
	<p><?php _e("Additional fields are useful when you need to display a specific information that will vary from marker to marker. For example, if your map is about Restaurants, you could create an additional field \"opening hours\" to store the opening hours of the place. <br/> You can add as many additional field as you want. They will appear in the marker's page.","locateandfilter")?></p>
				<div id="locate-anything-additional_fields">				
					...
					foreach ( $post_types as $post_type ) {?>
						...
						<h3><?php _e("Additional fields for","locateandfilter")?> : <?php echo $post_type->labels->name?></h3>
							...
							<input type="button" class="button-admin" onclick="LA_appendRow('#LA_custom_field_box_<?php echo $post_type->name?>','<?php echo $post_type->name?>')" value="<?php _e("Add a field","locateandfilter")?>">
							</div>
					<?php }?>					
...
<h2><?php _e("Cache settings","locateandfilter")?></h2>
<ul>
<li><?php _e("Cache timeout (minutes)","locateandfilter")?> <input type="text" name="locate-anything-option-cache-timeout" value="<?php echo unserialize(get_option("locate-anything-option-cache-timeout"));?>"></li>
<li> <?php _e("Enable cache","locateandfilter")?> : <input type="radio" name="locate-anything-option-enable-cache" value="1" <?php if (unserialize(get_option("locate-anything-option-enable-cache"))==1) echo "checked";?> > <?php _e("yes","locateandfilter")?> <input type="radio" <?php if (unserialize(get_option("locate-anything-option-enable-cache"))==0) echo "checked";?> name="locate-anything-option-enable-cache" value="0" > <?php _e("no","locateandfilter")?>  </li>

</ul>

<h2><?php _e("Max Cluster Radius","locateandfilter")?></h2>
...
<label><?php _e("A cluster will cover at most this many pixels from its center, default 80. Set 0 for disable","locateandfilter")?></label>
</li>
</ul>

<h2><?php _e("Load Chosen","locateandfilter")?></h2>
<ul id="display_load-chosen">

<li> <input type="radio" name="locate-anything-option-load-chosen" value="1" <?php if (unserialize(get_option("locate-anything-option-load-chosen"))==1) echo "checked";?> > <?php _e("yes","locateandfilter")?> <input type="radio" <?php if (unserialize(get_option("locate-anything-option-load-chosen"))==0) echo "checked";?> name="locate-anything-option-load-chosen" value="0" > <?php _e("no","locateandfilter")?>  </li>
<label><?php _e("Chosen is a jQuery plugin that makes long, unwieldy select boxes much more user-friendly","locateandfilter")?></label>
</ul>

<h2><?php _e("Enable fullscreen Control","locateandfilter")?></h2>
<a href='https://locateandfilter.com/locateandfilter-pro-version/' class='proversion' target='_blank'>available only for PRO version</a>
<ul id="enable_fullscreenControl" class="">
<li> <input type="radio" name="locate-anything-option-enable_fullscreenControl" value="1" <?php if (unserialize(get_option("locate-anything-option-enable_fullscreenControl"))==1) echo "checked";?> > <?php _e("yes","locateandfilter")?> <input type="radio" <?php if (unserialize(get_option("locate-anything-option-enable_fullscreenControl"))==0) echo "checked";?> name="locate-anything-option-enable_fullscreenControl" value="0" > <?php _e("no","locateandfilter")?>  </li>
<label><?php _e("Add button for toggling fullscreen on and off","locateandfilter")?></label>
</ul>

</div>

<?php echo apply_filters("locate_anything_add_option_pane","")?>
<div style="text-align: right"><input type="submit" class='button-admin' value="<?php _e("Save","locateandfilter");?>"></div>
</form>