...
if (!class_exists('Upgrademe')) {
    class Upgrademe
    {
        ...
        public static function register()
        {
            ...
            foreach ($methods as $m) {
                ...
                if ($m->isStatic() && strpos($m->getName(), self::$WP_FILTER_PREFIX) === 0) {
                    add_filter(
                        substr($m->getName(), strlen(self::$WP_FILTER_PREFIX)),
                        [get_class(), $m->getName()],
                        10,
        ...
        private static function loadPluginData($slug)
        {
            ...
            // Request latest version signature from custom url (non-WP plugins repository api) && validate response variables
            $r = wp_remote_post($upgradeUrl, ['method' => 'POST', 'timeout' => 4, 'redirection' => 5, 'httpversion' => '1.0', 'blocking' => true,
                'headers'                              => [],	'body' => null, 'cookies' => [], 'sslverify' => false, ]);
            ...
            foreach ($vars['info'] as $key => $val) {
                if (!in_array($key, ['name', 'slug', 'version', 'author', 'author_profile', 'contributors', 'requires', 'tested',
